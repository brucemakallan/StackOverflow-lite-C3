...
class Answer:
    ...
    def __init__(self, answer_id, question_id, user_id, answer, accepted, date_posted):
        self.id = answer_id
    ...
    def create(self, cur):
        """Create a new row"""
        try:
            sql = """INSERT INTO answers(question_id, user_id, answer_answer, answer_accepted, answer_date_posted) 
                                        VALUES(%s, %s, %s, %s, %s) RETURNING answer_id"""
            cur.execute(sql, (
    ...
    def update(self, cur):
        """Update one row"""
        try:
            sql = """UPDATE answers SET question_id = %s, answer_answer = %s, answer_accepted = %s 
                WHERE answer_id = %s"""
            cur.execute(sql, (
                str(self.question_id),
                self.answer,
